  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
        <h1 style={{ fontFamily: "var(--font-montserrat,'Montserrat',sans-serif)", fontSize: 22, fontWeight: 800, color: "var(--color-navy)", letterSpacing: "-0.03em" }}>Vélemények</h1>
        ...
        <div style={{ background: "rgba(13, 59, 102,0.04)", border: "1px solid rgba(13, 59, 102,0.14)", borderRadius: 12, padding: 28, marginBottom: 28 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div style={{ gridColumn: "1/-1" }}>
              <label style={{ fontSize: 12, color: "var(--color-muted)", display: "block", marginBottom: 6 }}>Idézet</label>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {items.map((t) => (
          <div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(13, 59, 102,0.03)", border: "1px solid rgba(13, 59, 102,0.10)", borderRadius: 10, padding: "16px 20px" }}>
            <div>
              <p style={{ fontSize: 14, fontWeight: 500, color: "var(--color-navy)" }}>{t.author}{t.role ? ` · ${t.role}` : ""}</p>
              <p style={{ fontSize: 12, color: "var(--color-muted)", marginTop: 2, maxWidth: 480 }}>{t.content}</p>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => openEdit(t)} style={{ background: "rgba(46,140,178,0.1)", color: "var(--color-accent-dark)", border: "none", padding: "6px 12px", borderRadius: 6, fontSize: 12, cursor: "pointer" }}>Szerkesztés</button>
